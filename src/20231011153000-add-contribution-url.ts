...
createMigration(module.exports, {
  up: async (db) => {
    await db.runSql(
      'ALTER TABLE license ADD contribution_url VARCHAR(255) DEFAULT NULL;',
    )
    ...
    for (const rowToAddLicenseUrl of rowsToAddLicenseUrl) {
      await db.runSql(
        `UPDATE license SET contribution_url = url, url = '${rowToAddLicenseUrl[1]}' WHERE id = ${rowToAddLicenseUrl[0]};`,
      )
    }
