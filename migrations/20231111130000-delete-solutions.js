...
function toSqlTuple(elements) {
  return "(" + elements.map((e) => JSON.stringify(e)).join(", ") + ")";
}

// src/utils/delete-utils.ts
async function deleteUnsupportedEntityTypes(args) {
  const { db, apiCache, unsupportedEntityTypes: unsupportedEntityTypes2 } = args;
  const entitiesToDelete = await db.runSql(`
    select entity.id as id
    from entity
    join type on entity.type_id = type.id
    where type.name in ${toSqlTuple(unsupportedEntityTypes2)}
  `);
  const revisionsToDelete = await db.runSql(`
  ...
  await db.runSql(
    `delete from type where name in ${toSqlTuple(unsupportedEntityTypes2)}`
  );
}
async function deleteUuids(db, apiCache, uuidsToDelete) {
  if (uuidsToDelete.length > 0) {
    const uuids = uuidsToDelete.map((uuid) => uuid.id);
    const eventLogsToDelete = await db.runSql(`
...
async function deleteEventLogs(db, apiCache, event_logs) {
...
// src/20231111130000-delete-solutions.ts
var unsupportedEntityTypes = ["text-solution"];
