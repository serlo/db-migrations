...
// src/20240229095500-add-expire-time-for-all-keys-without-expire-date.ts
...
createMigration(exports, {
  up: async () => {
    (0, import_assert.default)(process.env.REDIS_URL != null, "REDIS_URL must be defined");
    const oneWeek = 60 * 60 * 24 * 7;
    const migrationName = "add-expire-time-for-all-keys-without-expire-date";
    const slackLogger = new SlackLogger(migrationName);
    const redis = new import_ioredis2.Redis(process.env.REDIS_URL);
