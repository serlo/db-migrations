...
// src/20240205193700-merge-grouped-exercise-into-exercise-group.ts
...
var ChildContentDecoder = t2.type({
...
async function updateExerciseGroup(db, apiCache, parentId) {
  ...
  for (const edit of edits) {
    ...
    (0, import_assert.default)(parentContent != null, "Illegal state: Parent is null");
    ...
    if (parentContent.state.exercises != null)
      continue;
...
async function updateCurrentRevisionOfEntity({
  ...
  if (child.currentRevisionId != null && child.currentRevisionId > (parent.currentRevisionId ?? 0)) {
    await db.runSql(
...
async function loadEntityNode(db, entityId) {
  ...
  let childIds = [];
  if (entity.typeName === "text-exercise-group" /* ExerciseGroupType */) {
    childIds = await loadEntityChildrenIds({
      db,
      entityId,
      childType: "grouped-text-exercise" /* GroupedExerciseType */
    });
  }
...
function hasNonEmptyRevision(node) {
  if (node.value == null)
    return false;
...
var TypeName = /* @__PURE__ */ ((TypeName2) => {
  TypeName2["ExerciseGroupType"] = "text-exercise-group";
